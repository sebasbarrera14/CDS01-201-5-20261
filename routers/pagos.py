...
def procesar_pago(datos: PagoCreate, db: Session = Depends(get_db)):
    ...
    if pedido.estado != "pendiente":
        raise HTTPException(status_code=400, detail="El pedido no esta en estado pendiente")
    if datos.metodo not in METODOS_VALIDOS:
        raise HTTPException(status_code=400, detail=f"Metodo no valido. Opciones: {METODOS_VALIDOS}")
    ...
    pago = Pago(
        ...
        referencia_transaccion=f"REF-{datos.id_pedido}-{int(datetime.now().timestamp())}"
    )
    db.add(pago)
